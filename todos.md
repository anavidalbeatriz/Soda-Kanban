...
| Allow attach files to cards | Partial — S3 presigned API exists; no frontend UI in card modal | High |
...
| Allow deleting workspaces, cards, and boards | Partial — card/board DELETE APIs exist; workspace delete missing; no delete UI for cards/boards | Medium |

## Suggested follow-ups

| Issue | Status | Priority |
|-------|--------|----------|
| Card attachment UI (upload + view in card modal) | Not started | High |
| Card delete UI (wire existing `boardApi.deleteCard`) | Not started | High |
...
| Board delete UI (wire existing `DELETE /boards/{id}`) | Not started | Medium |
| Workspace rename API + UI | Not started | Medium |
| Workspace delete API + UI (with cascade/confirm UX) | Not started | Medium |
| Existing-user invite acceptance (API exists; no frontend) | Not started | Medium |
| S3 cleanup when attachment deleted (DB row removed but object may remain) | Not started | Low |
